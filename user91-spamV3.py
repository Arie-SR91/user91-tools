...
def logo():
  print ("""
                   ...
                   / __|_ __  __ _ __ __
                   \__ \ '_ \/ _` |  '  |
                   |___/ .__/\__,_|_|_|_| V3 By User91
                       |_|
...
def input():
  global nom
  nom = raw_input("%s[%s?%s] %sMasukkan nomor target (+62) : "%(pu,me,pu,pu))
  if len(nom) < 5:
...
def klikwa():
 ...
 if json.loads(tes.text)["message"] == 'OTP Sent':
  print "     %s[%s%s%s] [%s Sent %s] %sSuccess, spam %s from %s%s %ssended %s>> %sMau yang unlimited? %shttps://github.com/Arie-SR91/user91"%(pu,ku,"10",pu,hi,pu,pu,"wa",ku,"klikwa",hi,qu,pu,ku)
 else:
  print "     %s[%s%s%s] [%s Failed %s] %sFailed, spam %s from %s%s %snot sended %s>> %sMau yang unlimited? %shttps://github.com/Arie-SR91/user91"%(pu,ku,"10",pu,me,pu,pu,"wa",ku,"klikwa",me,qu,pu,ku)
def bakmikeraton():
